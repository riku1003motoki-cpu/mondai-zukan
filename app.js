...
function showAuthDialog() {
  ...
  document.querySelector("#signUp").onclick = async () => { if (!form.reportValidity()) return; const v = Object.fromEntries(new FormData(form)); const { error } = await supabase.auth.signUp({ email:v.email, password:v.password }); message.textContent = error ? `登録できませんでした：${error.message}` : "確認メールを送りました。メール内のリンクを開いた後、ログインしてください。"; };
